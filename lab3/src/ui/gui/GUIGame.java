...
import game.records.Pair;
import game.records.ScoreRecords;
import ui.GameViewController;
import ui.gui.CustomButton;
import ui.gui.CustomDialog;
import ui.gui.CustomPanel;
import ui.gui.UIDesigner;
...
import static time.Timer.*;
import static ui.gui.UIDesigner.DEFAULT_BACKGROUND;
import static util.TextUtils.getTimeString;
...
public class GUIGame extends JPanel implements GameViewController {
    ...
    private void loadContent() {
        try {
            ...
            JOptionPane.showMessageDialog(null, "I cant load images", "Error!", JOptionPane.ERROR_MESSAGE);
            System.exit(1);
        }
    }
    ...
    public void noticeOverGame() {
        if(Minesweeper.isMainController(this))
            JOptionPane.showMessageDialog(parent, "<html><h2>Game over</h2><i>play again!</i>");
    }

    @Override
    public void noticeWinGame() {
        if(Minesweeper.isMainController(this)) {
            JOptionPane.showMessageDialog(parent, "<html><h2>You win!</h2><i>Your score " +
            ...
            String name;
            do
                name = (String) JOptionPane.showInputDialog(parent, "Pleas, enter your name\nDon't use name with length more then 10", "Message", JOptionPane.QUESTION_MESSAGE, null, null, System.getProperty("user.name") == null ? "user" : System.getProperty("user.name"));
            ...
            parent.addWindowListener(new WindowAdapter()
            {
                ...
                public void windowClosing(WindowEvent e)
                {
                    if(Minesweeper.isGameStarted() && Minesweeper.isMainController(((GameViewController) ((JFrame) e.getWindow()).getContentPane()))) {
                        int a = JOptionPane.showConfirmDialog(parent,
                                "<html><h2>Are you sure?</h2><i>Do you want close the game?</i>",
                                "Message",
                                JOptionPane.YES_NO_OPTION,
                                JOptionPane.WARNING_MESSAGE);
